...
module.exports = {
  ...
  // ... все остальные роуты остаются без изменений ...
  "GET /": {
    "type": "view",
    "layout": "mainLayout",
    "reads": ["user", "receipt", "positions", "viewState"],
    "inject": { "pageContent": "cashierPage", "positionsList": "positionsList", "receipt": "receipt" },
    "auth": { "required": true, "failureRedirect": "/login" }
  },
  "GET /login": {
    "type": "view", "layout": "mainLayout",
    "inject": { "pageContent": "authLayout", "formContent": "loginForm" }
  },
  "GET /register": {
    "type": "view", "layout": "mainLayout",
    "inject": { "pageContent": "authLayout", "formContent": "registerForm" }
  },
  "POST /auth/login": {
  ...
  "GET /auth/logout": {
    "type": "action",
    "steps": [ { "auth:logout": true }, { "client:redirect": "'/login'" } ]
  },
  "POST /action/addItem": {
    "type": "action", "reads": ["positions", "receipt"], "writes": ["receipt"], "update": "receipt",
    "steps": [
  ...
  "POST /action/removeItem": {
      "type": "action", "reads": ["receipt"], "writes": ["receipt"], "update": "receipt",
      "steps": [
          // ★★★ КЛЮЧЕВОЕ ИСПРАВЛЕНИЕ: Используем `body.id`
  ...
  "POST /action/filterPositions": {
    "type": "action", "reads": ["positions", "viewState"], "writes": ["viewState"], "update": "positionsList",
  ...
  "POST /action/soft-refresh-receipt": {
    "type": "action", "reads": ["receipt"], "update": "receipt", "steps": []
  }
};